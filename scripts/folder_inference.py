...
# os.chdir('/home/nas2_userF/kangyeol/Project/webtoon2022/Paint-by-Example')
# print(os.getcwd())
...
def main():
    ...
    if os.path.isfile(opt.image_path) and os.path.isfile(opt.mask_path) and os.path.isfile(opt.reference_path):
        image_path_list = [opt.image_path]
        mask_path_list = [opt.mask_path]
        reference_path_list = [opt.reference_path]
    elif os.path.isdir(opt.image_path) and os.path.isdir(opt.mask_path) and os.path.isdir(opt.reference_path):
        image_path_list = [os.path.join(opt.image_path, f)for f in os.listdir(opt.image_path)]
        mask_path_list = [os.path.join(opt.mask_path, f)for f in os.listdir(opt.mask_path)]
        if opt.fix_reference:            
            reference_path_list = [os.path.join(opt.reference_path, f)for f in os.listdir(opt.reference_path)][0:1]
            reference_path_list = reference_path_list * len(image_path_list)

    assert len(image_path_list) == len(mask_path_list) == len(reference_path_list)
    
    def resize_viton_image(pil_img):
        w, h = pil_img.size
        if h > w:
            new_pil_img = pil_img.crop((0, 0, w, w))
        return new_pil_img.resize((512,512))
    
    precision_scope = autocast if opt.precision=="autocast" else nullcontext
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                for image_path, mask_path, reference_path in zip(image_path_list, mask_path_list, reference_path_list):

                    filename=os.path.basename(image_path)
                    img_p = Image.open(image_path).convert("RGB")
                    # img_p = resize_viton_image(img_p)                    
